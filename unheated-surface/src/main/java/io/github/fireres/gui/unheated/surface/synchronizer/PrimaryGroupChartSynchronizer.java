...
public class PrimaryGroupChartSynchronizer implements ChartSynchronizer<UnheatedSurfaceReport> {
    ...
    private void addThermocoupleTemperatureLines(LineChart<Number, Number> chart, UnheatedSurfaceReport report) {
        for (int i = 0; i < report.getThermocoupleTemperatures().size(); i++) {
            ...
            thermocoupleSeries.getNode().setId(ElementIds.FIRST_THERMOCOUPLE_GROUP_THERMOCOUPLE_TEMPERATURE_LINE);
        }
    }
    ...
    private void addThermocoupleBoundLine(LineChart<Number, Number> chart, UnheatedSurfaceReport report) {
        ...
        series.getNode().setId(ElementIds.FIRST_THERMOCOUPLE_GROUP_THERMOCOUPLE_MAX_TEMPERATURE_LINE);
    }
    ...
    private void addMeanBoundLine(LineChart<Number, Number> chart, UnheatedSurfaceReport report) {
        ...
        ChartUtils.addPointsToSeries(series, report.getMaxAllowedMeanTemperature());
        chart.getData().add(series);
        series.getNode().setId(ElementIds.FIRST_THERMOCOUPLE_GROUP_MEAN_BOUND_TEMPERATURE_LINE);
    }
